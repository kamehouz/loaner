/* Dinio — Loan form modal + columns manager drawer */
...
function Field({ f, value, onChange, error, columnName, onManageStatuses }) {
  ...
  const locked = f.core;
  const common = { className: 'inp' + (error ? ' err' : ''), value: value || '', onChange: e => onChange(f.key, e.target.value) };
  let control;
  if (f.type === 'textarea') control = <textarea className={'area' + (error ? ' err' : '')} value={value || ''} onChange={e => onChange(f.key, e.target.value)} />;
  else if (f.type === 'date') control = <input type="date" {...common} />;
  else if (f.type === 'money') control = <input type="number" min="0" step="1000" placeholder="0" {...common} />;
  ...
  else if (f.type === 'status') control = (
    <select className="sel" value={value} onChange={e => onChange(f.key, e.target.value)}>
      {Core.orderedStatuses().map(s => { const i = Core.statusInfo(s.id); return <option key={s.id} value={s.id}>{i.code} · {i.label}</option>; })}
    </select>
  );
  else control = <input type="text" placeholder={f.placeholder || ''} {...common} />;
    ...
    <div className={'fld' + (f.type === 'textarea' ? ' full' : '') + (locked ? ' locked' : '')}>
      <label>
        ...
        {f.type === 'status' && onManageStatuses &&
          <button type="button" className="edit-statuses" onClick={onManageStatuses}><Icon name="cog" size={12} />Edit statuses</button>}
      </label>
      {control}
...
function LoanForm({ loan, isNew, columns, onSave, onDelete, onClose, onManageStatuses }) {
  const [form, setForm] = useStateF({ ...loan });
  ...
  const set = (k, v) => { setForm(p => ({ ...p, [k]: v })); if (errs[k]) setErrs(p => ({ ...p, [k]: null })); };

  // which optional columns are still present (not removed) — drives which optional fields show
  ...
  const validate = () => {
    ...
    if (form.amount === '' || form.amount == null || isNaN(Number(form.amount)) || Number(form.amount) <= 0) e.amount = 'Enter a loan amount';
    if (Core.statusInfo(form.status).role === 'closed' && !form.close) e.close = 'Close date is required for a closed loan';
    setErrs(e); return Object.keys(e).length === 0;
  };
  const save = () => { if (validate()) onSave({ ...form, amount: Number(form.amount) || 0 }); };
      ...
      <div className="modal" onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="form-grid">
            {visibleFields.map(f => (
              <Field key={f.key} f={f} value={form[f.key]} onChange={set} error={errs[f.key]} columnName={colName(f.key)} onManageStatuses={onManageStatuses} />
            ))}
            <div className="calc-box">
              <div className="ck"><span className="k">2% commission</span><span className="v money">{willClose ? Core.money(amt * Core.CONFIG.closePct) : '—'}</span></div>
              <div className="ck"><span className="k">0.5% monthly trail</span><span className="v money">{willClose ? Core.money(amt * Core.CONFIG.trailPct) : '—'}</span></div>
              <div className="note">Auto-calculated — never entered. {willClose ? 'Active for the life of the loan.' : 'Shown once status is Closed.'}</div>
            </div>
          </div>
        </div>
        <div className="modal-foot">
          {!isNew && <button className="btn danger grow" onClick={() => onDelete(form)}><Icon name="trash" size={15} />Delete</button>}
          {isNew && <span className="grow" />}
          <button className="btn ghost" onClick={onClose}>Cancel</button>
          <button className="btn primary" onClick={save}><Icon name="check" size={15} />{isNew ? 'Add loan' : 'Save changes'}</button>
        </div>
      </div>
    </div>
  );
...
function ColumnsTab({ columns, setColumns }) {
  ...
  const onDrop = (i) => {
    ...
    const next = [...columns]; const [m] = next.splice(drag, 1); next.splice(i, 0, m);
    setColumns(next); setDrag(null);
  };
  return (
    <React.Fragment>
      <div className="col-group-label"><Icon name="check" size={13} style={{ color: 'var(--green)' }} /> Locked · core data points</div>
      {LOCKED_COLS.map((c, i) => (
      ...
      {columns.map((c, i) => (
        <div key={c.key} className="col-row" draggable
          ...
          <button className={'switch' + (c.on ? ' on' : '')} onClick={() => toggle(i)} title={c.on ? 'Visible' : 'Hidden'}><span className="knob" /></button>
          <button className="icon-btn danger" onClick={() => remove(i)} title="Remove column entirely"><Icon name="trash" size={15} /></button>
        </div>
      ))}
      {columns.length === 0 && <div className="muted" style={{ fontSize: 13, padding: '8px 4px' }}>All optional columns removed — only core data points remain.</div>}
    </React.Fragment>
  );
}
...
function StatusesTab({ statuses, setStatuses }) {
  ...
  const onDrop = (id) => {
    ...
    setStatuses(norm([...list, closed, dead].filter(Boolean))); setDrag(null);
  };
    ...
    <React.Fragment>
      ...
      {progress.map((s) => (
        <div key={s.id} className="col-row" draggable
          ...
          <button className="icon-btn danger" onClick={() => remove(s.id)} title="Remove stage"
            disabled={progress.length <= 1} style={{ opacity: progress.length <= 1 ? .3 : 1, pointerEvents: progress.length <= 1 ? 'none' : 'auto' }}>
      ...
      <div className="col-group-label"><Icon name="check" size={13} style={{ color: 'var(--green)' }} /> Locked roles · drive commissions &amp; alerts</div>
      {closed && (
      ...
      {dead && (
        <div className="col-row locked dead-row">
      ...
      <p className="drawer-hint">Rename these freely (e.g. “Closed” → “Funded”), but their role is locked — that’s how the app knows which loans earn commission and which to drop from the pipeline.</p>
    </React.Fragment>
  );
}

function ColumnsDrawer({ columns, setColumns, statuses, setStatuses, onClose, initialTab = 'columns' }) {
  const [tab, setTab] = useStateF(initialTab);
  useEffectF(() => {
    const h = e => { if (e.key === 'Escape') onClose(); };
    window.addEventListener('keydown', h); return () => window.removeEventListener('keydown', h);
  }, []);
      ...
      <div className="drawer">
        <div className="drawer-head">
          ...
          <p>{tab === 'columns'
            ...
            : 'Rename, add, remove or reorder your pipeline stages. The “Closed” and “Dead” roles are locked so commissions and alerts keep working.'}</p>
...
Object.assign(window, { LoanForm, ColumnsDrawer, ALL_FIELDS });
