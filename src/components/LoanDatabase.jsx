...
export function LoanDatabase({ loans, columns, onOpen, onAdd, onManage, search, setSearch, filter, setFilter }) {
                ...
                {visCols.map(c => (
                  <th key={c.key}>
                    <span className="col-head">
                      {c.name}
                      <span className="col-cog" onClick={e => { e.stopPropagation(); onManage('columns') }} title="Manage columns">
                ...
                {filtered.map(l => (
                  <tr key={l.id} className="clickable" onClick={() => onOpen(l)}>
                    ...
                    <td className="num money">
                      {Core.isClosed(l) ? Core.money(Core.closingComm(l)) : <span className="dash-cell">—</span>}
                    </td>
                    <td className="num money">
                      {Core.isClosed(l) ? Core.money(Core.monthlyTrail(l)) : <span className="dash-cell">—</span>}
                    </td>
                    <td>{l.close ? Core.fmtDate(l.close) : <span className="dash-cell">—</span>}</td>
                    <td>
                      <div className="row-actions">
                        <button className="icon-btn" onClick={e => { e.stopPropagation(); onOpen(l) }} title="Edit">
                          <Icon name="edit" size={15} />
                        </button>
                        <button className="icon-btn danger" onClick={e => { e.stopPropagation(); onOpen(l, true) }} title="Delete">
          ...
          <div className="empty">
            ...
            {!loans.length && (
              <button className="btn primary" style={{ marginTop: 16 }} onClick={onAdd}>
                <Icon name="plus" size={16} />Add new loan
              </button>
            )}
          </div>
        )}
