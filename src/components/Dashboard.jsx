...
import { Kpi, PipelineBars, StatusBadge } from './Shared.jsx'
    ...
    <div className="card trail card-pad" style={{ marginBottom: 18 }}>
      <div className="trail-head">
        ...
        <div className="month-nav">
          <button className="icon-btn" onClick={() => setMk(Core.shiftMonth(mk, -1))} title="Previous month">
            <Icon name="left" size={16} />
          </button>
          <span className="lbl">{Core.monthLabel(mk)}</span>
          <button
            className="icon-btn"
            onClick={() => setMk(Core.shiftMonth(mk, 1))}
            disabled={atCurrent}
            style={{ opacity: atCurrent ? .35 : 1, pointerEvents: atCurrent ? 'none' : 'auto' }}
            title="Next month"
          >
      ...
      {closed.length > 0 ? (
        <>
          <div className="trail-summary">
            ...
            <div className="meter">
              <div style={{ width: (total ? Math.round(gotAmt / total * 100) : 0) + '%' }} />
            </div>
    ...
    <div className="card alerts">
      <div className="card-pad" style={{ paddingBottom: 6 }}>
        <div className="section-title">
          <Icon name="dollar" size={17} style={{ color: 'var(--green)' }} />
          Commission summary
          <span className="hint">· {closed.length} closed loan{closed.length !== 1 ? 's' : ''}</span>
        </div>
      </div>
      {closed.length ? (
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr>
              ...
              <th>Close date</th>
              <th className="num">2% commission</th>
              <th className="num">Trail / mo</th>
        ...
        <div className="empty" style={{ padding: '40px 20px' }}>
          <Icon name="dollar" size={32} className="ic" />
          <h3>No closed loans yet</h3>
          <div>Commissions appear here once a loan is marked Closed.</div>
        </div>
      )}
    </div>
  )
...
export function Dashboard({ loans, confirm, setConfirm, onOpen }) {
    ...
    <div>
      ...
      <div className="dash-cols">
        <div className="card card-pad">
          <div className="section-title">
            <Icon name="layers" size={17} style={{ color: 'var(--blue)' }} /> Pipeline by stage
          </div>
      ...
      <div className="card alerts">
        <div className="card-pad">
          <div className="section-title">
            <Icon name="flag" size={17} style={{ color: 'var(--amber)' }} />
            Follow-up alerts
            <span className="hint">· untouched &gt; {Core.CONFIG.followDays} business days</span>
          </div>
          {alerts.length ? (
            ...
            <div className="all-clear">
              <Icon name="check" size={18} /> All caught up — every active loan was followed up recently.
            </div>
          )}
        </div>
      </div>
